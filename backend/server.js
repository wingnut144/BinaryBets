...
app.post('/api/markets/:marketId/resolve', async (req, res) => {
  ...
  try {
    ...
    const winningBets = await pool.query(
      \`SELECT b.*, u.email, u.username FROM bets b
       JOIN users u ON b.user_id = u.id
       WHERE b.market_id = $1 AND b.market_option_id = $2\`,
    ...
    const result = await pool.query(\`
      SELECT 
        ...
        (SELECT COUNT(*) FROM bets b 
         JOIN markets m ON b.market_id = m.id 
         WHERE b.user_id = users.id AND m.resolved = true AND m.winning_option_id = b.market_option_id) as wins
      FROM users
