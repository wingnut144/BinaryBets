...
// Send loser notification (optional)
export async function sendLoserEmail(user, bet, market) {
  const transporter = createTransporter();
  ...
  const mailOptions = {
    from: `"BinaryBets" <${process.env.EMAIL_USER}>`,
    to: user.email,
    subject: 'Bet Resolved - Better Luck Next Time',
    html: `
      <!DOCTYPE html>
      <html>
      <head>
      ...
      <body>
        <div class="container">
          ...
          <div class="content">
            <h2>Hi @${user.username},</h2>
            <p>Your bet has been resolved. Unfortunately, this one didn't go your way.</p>
            ...
            <p>Don't give up! Every great bettor has losses. Check out our active bets and try again!</p>
            <p><strong>Current Balance:</strong> $${parseFloat(user.balance).toFixed(2)}</p>
          </div>
          <div class="footer">
            <p>Visit <a href="${process.env.FRONTEND_URL || 'http://localhost:3000'}">BinaryBets</a> to place more bets!</p>
            <p>&copy; 2025 BinaryBets. All rights reserved.</p>
